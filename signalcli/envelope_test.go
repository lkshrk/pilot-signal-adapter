...
func load(t *testing.T, name string) signalcli.Frame {
	t.Helper()
	b, err := os.ReadFile(filepath.Join("testdata", name))
	if err != nil {
		t.Fatalf("read fixture: %v", err)
	}
	f, err := signalcli.ParseFrame(b)
	if err != nil {
...
func TestEnvelopeKind(t *testing.T) {
	...
	tests := []struct {
		fixture string
		want    signalcli.FrameKind
		group   bool
	}{
		{"group_data_message.json", signalcli.KindData, true},
		{"typing_message.json", signalcli.KindTyping, false},
		{"receipt_message.json", signalcli.KindReceipt, false},
		{"poll_vote.json", signalcli.KindPollVote, true},
		{"poll_create.json", signalcli.KindPollCreate, true},
	}
...
func TestPollVoteIsNotMistakenForAMessage(t *testing.T) {
	...
	if got := f.Envelope.Kind(); got == signalcli.KindData {
...
func TestParseFrameRejectsGarbage(t *testing.T) {
	...
	if _, err := signalcli.ParseFrame([]byte("not json")); err == nil {
...
func TestUnknownKindForEmptyEnvelope(t *testing.T) {
	...
	f, err := signalcli.ParseFrame([]byte(`{"envelope":{},"account":"+490000000001"}`))
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	if got := f.Envelope.Kind(); got != signalcli.KindUnknown {
		t.Errorf("Kind() = %q, want %q", got, signalcli.KindUnknown)
	}
}
