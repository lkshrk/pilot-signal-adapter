...
func quietLogger() signalcli.Option {
	return signalcli.WithLogger(slog.New(slog.DiscardHandler))
}
...
func TestRunDeliversFrames(t *testing.T) {
	...
	c, err := signalcli.New(srv.URL, testAccount, quietLogger())
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	got := make(chan signalcli.Frame, 1)
	go func() {
		_ = c.Run(ctx, func(f signalcli.Frame) {
			select {
			case got <- f:
			default:
			}
	...
	select {
	case f := <-got:
		if f.Envelope.Kind() != signalcli.KindData {
...
func TestRunReconnectsAfterDrop(t *testing.T) {
	...
	c, err := signalcli.New(srv.URL, testAccount, quietLogger(),
		signalcli.WithBackoff(time.Millisecond, 10*time.Millisecond))
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	go func() {
		_ = c.Run(ctx, func(signalcli.Frame) {
			select {
			case got <- struct{}{}:
			default:
			}
...
func TestRunReportsReconnects(t *testing.T) {
	...
	var attempts []int
	c, err := signalcli.New(srv.URL, testAccount, quietLogger(),
		signalcli.WithBackoff(time.Millisecond, 5*time.Millisecond),
		signalcli.WithOnReconnect(func(attempt int, _ time.Duration, _ error) {
			mu.Lock()
...
func TestRunStopsOnFatalClose(t *testing.T) {
	...
	err = c.Run(ctx, func(signalcli.Frame) {})
	if err == nil {
		t.Fatal("fatal close did not stop the loop")
	}
	var fatal *signalcli.FatalCloseError
...
func TestRunStopsOnRejectedHandshake(t *testing.T) {
	...
	if err := c.Run(ctx, func(signalcli.Frame) {}); err == nil {
...
func TestNewValidatesInput(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := signalcli.New(tc.base, tc.account); err == nil {
...
func TestWithBackoffRejectsInvalidBounds(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := signalcli.New("http://host", testAccount,
				signalcli.WithBackoff(tc.lo, tc.hi)); err != nil {
				t.Fatalf("New: %v", err)
			}
		})
	}
