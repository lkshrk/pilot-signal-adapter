...
import (
	...
	"github.com/lkshrk/pilot-signal-adapter/signalcli"
)
...
func TestAllowlistIsDefaultDeny(t *testing.T) {
	...
	tests := []struct {
		name  string
		allow *signalcli.GroupAllowlist
		want  bool
	}{
		{"allowlisted group", signalcli.NewGroupAllowlist(fixtureGroup), true},
		{"other group only", signalcli.NewGroupAllowlist("some-other-group"), false},
		{"empty allowlist", signalcli.NewGroupAllowlist(), false},
		{"empty string is not a group", signalcli.NewGroupAllowlist(""), false},
...
func TestOnlyDataFramesBecomeCommands(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.fixture, func(t *testing.T) {
			t.Parallel()
			cmd, ok := signalcli.CommandFrom(allow, load(t, tc.fixture).Envelope)
			if ok != tc.want {
...
func TestVoteFromBindsAndAttributes(t *testing.T) {
	...
	vote, ok := signalcli.VoteFrom(allow, env)
	if !ok {
...
func TestVoteFromRejectsNonVotes(t *testing.T) {
	...
	for _, fixture := range []string{"group_data_message.json", "poll_create.json"} {
		if _, ok := signalcli.VoteFrom(allow, load(t, fixture).Envelope); ok {
...
func TestVoteFromDeniesForeignGroup(t *testing.T) {
	...
	allow := signalcli.NewGroupAllowlist("some-other-group")
	if _, ok := signalcli.VoteFrom(allow, load(t, "poll_vote.json").Envelope); ok {
...
func TestGroupRecipientConvertsReceivedForm(t *testing.T) {
	...
	got := signalcli.GroupRecipient(received)
	if got == received {
...
func TestGroupRecipientIsIdempotent(t *testing.T) {
	...
	once := signalcli.GroupRecipient("Km7Q/DSD6h3XlGjsZ5BSZ+nKbdukhEe/2rYzaJYwmrw=")
	if twice := signalcli.GroupRecipient(once); twice != once {
		t.Errorf("double conversion changed the recipient: %q -> %q", once, twice)
	}
	if got := signalcli.GroupRecipient(""); got != "" {
...
func TestVoteFromRequiresPollAuthor(t *testing.T) {
	...
	f, err := signalcli.ParseFrame([]byte(raw))
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	if _, ok := signalcli.VoteFrom(signalcli.NewGroupAllowlist(fixtureGroup), f.Envelope); ok {
...
func TestAllowlistAcceptsEitherGroupEncoding(t *testing.T) {
	...
	sendForm := signalcli.GroupRecipient(fixtureGroup)
	...
	for _, configured := range []string{fixtureGroup, sendForm} {
		if !signalcli.NewGroupAllowlist(configured).Allows(received) {
...
func TestGroupIDRoundTripsWithGroupRecipient(t *testing.T) {
	...
	if got := signalcli.GroupID(signalcli.GroupRecipient(raw)); got != raw {
		t.Errorf("round trip = %q, want %q", got, raw)
	}
	// A malformed send-form value must degrade to itself, not to empty.
	if got := signalcli.GroupID("group.!!!not-base64!!!"); got == "" {
