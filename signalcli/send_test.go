...
func newSender(t *testing.T, srv *httptest.Server, opts ...signalcli.SenderOption) *signalcli.Sender {
	t.Helper()
	s, err := signalcli.NewSender(srv.URL, testAccount, opts...)
	if err != nil {
...
func TestSendChunkedStopsAtFirstFailure(t *testing.T) {
	...
	s := newSender(t, srv, signalcli.WithMaxMessageLength(5))
...
func TestSendChunkedSplitsOnLines(t *testing.T) {
	...
	srv, got := apiServer(t, http.StatusCreated, `{"timestamp":"1"}`)
	s := newSender(t, srv, signalcli.WithMaxMessageLength(12))
...
func TestSendChunkedHardCutsAnOversizedLine(t *testing.T) {
	...
	srv, got := apiServer(t, http.StatusCreated, `{"timestamp":"1"}`)
	s := newSender(t, srv, signalcli.WithMaxMessageLength(4))
...
func TestNewSenderValidates(t *testing.T) {
	...
	if _, err := signalcli.NewSender("", testAccount); err == nil {
		t.Error("empty base url accepted")
	}
	if _, err := signalcli.NewSender("http://host", ""); err == nil {
...
func TestWithMaxMessageLengthIgnoresNonPositive(t *testing.T) {
	...
	s, err := signalcli.NewSender("http://host", testAccount, signalcli.WithMaxMessageLength(0))
	if err != nil {
		t.Fatalf("NewSender: %v", err)
	}
	if s.MaxMessageLength() != signalcli.DefaultMaxMessageLength {
...
func TestSendTextSurfacesPerRecipientFailures(t *testing.T) {
	...
	var de *signalcli.DeliveryError
	if !errors.As(err, &de) {
...
func TestCreatePollPostedButUnidentified(t *testing.T) {
	...
	var pe *signalcli.PostedUnidentifiedError
	if !errors.As(err, &pe) {
		t.Fatalf("err = %v, want PostedUnidentifiedError", err)
	}
	var ae *signalcli.APIError
	if errors.As(err, &ae) {
...
func TestSendChunkedRefusesToFloodAGroup(t *testing.T) {
	...
	srv, got := apiServer(t, http.StatusCreated, `{"timestamp":"1"}`)
	s := newSender(t, srv, signalcli.WithMaxMessageLength(10))
